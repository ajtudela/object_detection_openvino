...
ObjectDetectionOpenvino::ObjectDetectionOpenvino(ros::NodeHandle& node, ros::NodeHandle& node_private) : node_(node), nodePrivate_(node_private), 
	...
	boundingBoxesPublisher_ = node_.advertise<object_detection_openvino::BoundingBoxArray>("bounding_boxes", 1);
	detectionImagePublisher_ = imageTransport_.advertise("detection_image", 1);
	markerPublisher_ = node_.advertise<visualization_msgs::MarkerArray>("detection_markers", 1);
	boundingBoxes3dPublisher_ = node_.advertise<object_detection_openvino::BoundingBox3dArray>("bounding_boxes3d", 1);
